...
public class GetTest {
  ...
  public async Task GetVertexTest() {
    var tuningJob = await vertexClient.Tunings.GetAsync("tuningJobs/4028554580617854976", null);

    Assert.IsNotNull(tuningJob);
  }

  [TestMethod]
  public async Task GetGeminiTest() {
    var ex = await Assert.ThrowsExceptionAsync<ClientError>(async () => {
      await geminiClient.Tunings.GetAsync("tuningJobs/4028554580617854976", null);
    });
